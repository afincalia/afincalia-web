...
import { db, notifyLead, ready, NOTIFICATION_RETRY_WINDOW_MS } from '../../lib/leads';
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const expired = await db(`commercial_leads?select=id&notified_at=is.null&created_at=lte.${cutoff}&limit=1`, undefined, 'GET');
    ...
    const rows = await db(`commercial_leads?notified_at=is.null&created_at=gt.${cutoff}&order=created_at.asc&limit=5`, undefined, 'GET');
    let sent = 0;
