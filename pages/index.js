...
const FB=[
  ...
  {k:["moroso","no paga","deuda","cuota"],t:"El administrador puede reclamar sin abogado para deudas bajo 2.000€.",l:"Art. 21 LPH — procedimiento monitorio",tp:"GESTIÓN"},
...
const TL={URGENTE:"🔴 Urgente",ALTA:"🟠 Alta",NORMAL:"🟢 Registrado",GESTIÓN:"🔵 En gestión",INFO:"ℹ️ Info"};
  ...
  return(
    <Link href="/" style={{textDecoration:"none",display:"flex",alignItems:"center",gap:8}}>
      <div style={{display:"flex",flexDirection:"column",gap:2}}>
        <div style={{display:"flex",gap:2}}>{[0,1,2].map(i=><div key={i} style={{width:5,height:5,borderRadius:"50%",background:V.rojo}}/>)}</div>
        <div style={{display:"flex",gap:2}}>{[0,1,2,3,4].map(i=><div key={i} style={{width:5,height:5,borderRadius:"50%",background:V.rojo}}/>)}</div>
      </div>
...
// ─── NAVBAR ──────────────────────────────────────────────────
function Navbar({onTrial, active=""}){
  ...
  return(
    <nav style={{position:"sticky",top:0,zIndex:100,background:sc?"rgba(250,250,248,.98)":"rgba(250,250,248,.94)",backdropFilter:"blur(16px)",borderBottom:`1px solid ${sc?V.borde:"transparent"}`,transition:"all .3s"}}>
      <div style={{maxWidth:1060,margin:"0 auto",padding:"0 20px",height:60,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <div className="dnav">
          {links.map(([l,href])=>(
            <Link key={href} href={href} style={{fontSize:14,color:active===href?V.verde:V.sub,padding:"7px 12px",textDecoration:"none",fontWeight:active===href?700:400,borderRadius:8}}>
              {l}
            </Link>
          ))}
          <button onClick={onTrial} style={{marginLeft:12,background:V.verde,color:V.w,border:"none",borderRadius:100,padding:"10px 22px",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"system-ui",boxShadow:"0 2px 12px rgba(26,107,66,.25)"}}>
            14 días gratis
          </button>
        </div>
      ...
      {menu&&(
        <div style={{background:V.w,borderTop:`1px solid ${V.borde}`,padding:"16px 20px 24px",boxShadow:"0 12px 32px rgba(0,0,0,.1)"}}>
          ...
          {[["🤖","Cómo funciona","/como-funciona"],["📄","Actas IA","/actas"],["🛡️","RGPD","/rgpd"]].map(([ic,l,href])=>(
            <Link key={href} href={href} onClick={()=>setMenu(false)} style={{display:"flex",alignItems:"center",gap:12,fontSize:15,color:V.tx,padding:"12px 0",borderBottom:`1px solid ${V.borde}`,textDecoration:"none",fontWeight:500}}>
              <span style={{fontSize:18,width:26}}>{ic}</span>{l}
            </Link>
          ))}
          ...
          <button onClick={()=>{setMenu(false);onTrial();}} style={{width:"100%",marginTop:16,background:V.verde,color:V.w,border:"none",borderRadius:100,padding:"14px 0",fontSize:16,fontWeight:700,cursor:"pointer",fontFamily:"system-ui"}}>
            Empezar 14 días gratis →
          </button>
          <a href={WA} target="_blank" rel="noopener noreferrer" style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginTop:10,background:"#25D366",color:V.w,borderRadius:100,padding:"13px 0",fontSize:15,fontWeight:700,textDecoration:"none"}}>
    ...
    <footer style={{background:V.alt,padding:"48px 20px 32px",borderTop:`1px solid ${V.borde}`}}>
      <div style={{maxWidth:980,margin:"0 auto"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:40,marginBottom:40}}>
          <div>
            <Logo/>
            <p style={{marginTop:16,fontSize:14,color:V.mut,lineHeight:1.75}}>El empleado digital para administradores de fincas en España. LPH · RGPD · 24/7</p>
          </div>
          <div>
            <p style={{fontWeight:700,color:V.sub,marginBottom:16,fontSize:12,textTransform:"uppercase",letterSpacing:".1em"}}>Producto</p>
            {[["Cómo funciona","/como-funciona"],["Actas IA","/actas"],["Guardián RGPD","/rgpd"],["Precios","/precios"]].map(([l,h])=>(
              <div key={l} style={{marginBottom:10}}><Link href={h} style={{fontSize:14,color:V.sub,textDecoration:"none"}}>{l}</Link></div>
            ))}
          </div>
          <div>
            <p style={{fontWeight:700,color:V.sub,marginBottom:16,fontSize:12,textTransform:"uppercase",letterSpacing:".1em"}}>Legal</p>
            {[["Aviso legal","/aviso-legal"],["Política de privacidad","/politica-privacidad"],["Política de cookies","/politica-cookies"],["Condiciones de uso","/condiciones-uso"],["Contrato DPA","/contrato-dpa"]].map(([l,h])=>(
              <div key={l} style={{marginBottom:10}}><Link href={h} style={{fontSize:14,color:V.mut,textDecoration:"none"}}>{l}</Link></div>
            ))}
          </div>
          <div>
        ...
        <div style={{borderTop:`1px solid ${V.borde}`,paddingTop:20,display:"flex",flexWrap:"wrap",gap:10,justifyContent:"space-between",fontSize:12,color:V.mut}}>
          <p>© 2026 AfincalIA · No sustituye asesoramiento jurídico profesional.</p>
          <p>RGPD · LOPDGDD · LSSI-CE · Datos en la UE (Alemania)</p>
        </div>
      </div>
    </footer>
  );
...
function Bubble({m,hora}){
  if(m.from==="user")return(
    <div style={{alignSelf:"flex-end",maxWidth:"84%"}}>
      <div style={{background:"#DCF8C6",padding:"9px 13px",borderRadius:12,borderTopRightRadius:2,fontSize:14,lineHeight:1.55,color:"#111",boxShadow:"0 1px 2px rgba(0,0,0,.08)"}}>
        {m.text}<div style={{fontSize:10,color:"#999",textAlign:"right",marginTop:4}}>{hora} ✓✓</div>
      </div>
    </div>
  );
  return(
    <div style={{alignSelf:"flex-start",maxWidth:"90%"}}>
      <div style={{background:V.w,padding:"9px 13px",borderRadius:12,borderTopLeftRadius:2,fontSize:14,lineHeight:1.65,color:"#111",boxShadow:"0 1px 2px rgba(0,0,0,.08)"}}>
        <div>{m.texto}</div>
        {m.ley&&(
          <a href="https://www.boe.es/buscar/act.php?id=BOE-A-1960-10906" target="_blank" rel="noopener noreferrer"
  ...
  const send=async txt=>{
    ...
    try{
      ...
      if(res.ok){
        ...
        if(d.text){
          const g=k=>{const m=d.text.match(new RegExp(k+":\\s*(.+)"));return m?.[1]?.trim()||null;};
          const texto=g("TEXTO"),ley=g("LEY"),url=g("URL"),tipo=g("TIPO");
          if(texto)r={texto,ley:ley==="ninguna"?null:ley,url:url==="ninguna"?null:url,tipo:tipo||"INFO"};
        }
      }
    }catch(e){}
    if(!r)r=getFB(text);
  ...
  return(
    <div style={{borderRadius:20,overflow:"hidden",boxShadow:"0 32px 80px rgba(0,0,0,.18)",maxWidth:360,width:"100%",margin:"0 auto"}}>
      <div style={{background:"#075E54",padding:"12px 16px",display:"flex",alignItems:"center",gap:11}}>
        ...
        <div>
          <div style={{color:V.w,fontWeight:700,fontSize:14}}>AfincalIA</div>
      ...
      {msgs.length<=1&&(
        <div style={{background:"#F0F2F5",padding:"8px 10px",display:"flex",flexWrap:"wrap",gap:6}}>
          {["El ascensor no funciona","Gotera en el portal","¿Puedo instalar un cargador?","Mi vecino hace ruido"].map((s,i)=>(
            <button key={i} onClick={()=>send(s)} style={{background:V.w,border:"1px solid #ddd",borderRadius:100,padding:"5px 12px",fontSize:11,color:"#333",cursor:"pointer",fontFamily:"system-ui"}}>{s}</button>
          ))}
        </div>
      )}
      <div style={{background:"#F0F2F5",padding:"9px 10px",display:"flex",gap:8,alignItems:"center"}}>
        <input value={inp} onChange={e=>setInp(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Escribe tu consulta..." disabled={busy}
          style={{flex:1,border:"none",borderRadius:22,padding:"10px 15px",fontSize:14,background:V.w,outline:"none",fontFamily:"system-ui"}}/>
        <button onClick={()=>send()} disabled={busy||!inp.trim()}
          style={{width:40,height:40,borderRadius:"50%",background:inp.trim()&&!busy?"#128C7E":"#ccc",border:"none",cursor:inp.trim()&&!busy?"pointer":"default",color:V.w,fontSize:18,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>↑</button>
      </div>
    </div>
  );
}
...
function Modal({onClose}){
  ...
  const ok=Object.values(form).every(v=>v.trim());
  const submit=async()=>{
    if(!ok)return;
    setStep("loading");
    try{await fetch("/api/registro",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...form,plan:"Profesional",trial:14})});}catch(e){}
    setStep("success");
  };
      ...
      <div style={{background:V.w,borderRadius:20,width:"100%",maxWidth:480,maxHeight:"92vh",overflowY:"auto",position:"relative",boxShadow:"0 24px 64px rgba(0,0,0,.28)"}}>
        ...
        {step==="loading"&&(
          <div style={{textAlign:"center",padding:"60px 24px"}}>
            <div style={{width:44,height:44,border:`3px solid ${V.pale}`,borderTop:`3px solid ${V.verde}`,borderRadius:"50%",animation:"spin 1s linear infinite",margin:"0 auto 18px"}}/>
            <p style={{color:V.sub,fontSize:15}}>Activando tu prueba gratis...</p>
          </div>
        )}
        ...
        {step==="success"&&(
          <div style={{textAlign:"center",padding:"44px 24px"}}>
            ...
            <h3 style={{fontFamily:"Georgia,serif",fontSize:24,marginBottom:12}}>¡Todo listo!</h3>
            <p style={{fontSize:15,color:V.sub,lineHeight:1.75,marginBottom:8}}>
              En menos de 2 minutos recibirás un WhatsApp en <strong>{form.telefono}</strong> con las instrucciones de activación.
            </p>
            <p style={{fontSize:13,color:V.mut,marginBottom:28}}>14 días con el plan Profesional completo. Sin tarjeta.</p>
            <button onClick={onClose} style={{background:V.verde,color:V.w,border:"none",borderRadius:100,padding:"13px 32px",fontWeight:700,cursor:"pointer",fontSize:15,fontFamily:"system-ui"}}>Perfecto ✓</button>
          </div>
        )}

        {step==="form"&&(
          <div style={{padding:"28px 24px 24px"}}>
            <div style={{background:V.pale,borderRadius:12,padding:"14px 16px",marginBottom:20,border:`1px solid ${V.borde}`}}>
              <div style={{fontSize:13,fontWeight:700,color:V.verde,marginBottom:4}}>✓ 14 días gratis — Plan Profesional completo</div>
              <div style={{fontSize:12,color:V.sub}}>Actas IA, expedientes, morosidad y guardián RGPD. Sin tarjeta.</div>
            </div>
            <input style={si} placeholder="Nombre y apellidos *" value={form.nombre} onChange={up("nombre")} autoComplete="name"/>
            <input style={si} placeholder="Email profesional *" type="email" value={form.email} onChange={up("email")} autoComplete="email"/>
            <input style={si} placeholder="Teléfono (recibirás WhatsApp) *" type="tel" value={form.telefono} onChange={up("telefono")} autoComplete="tel"/>
            <input style={si} placeholder="Nombre del despacho *" value={form.despacho} onChange={up("despacho")}/>
            <input style={si} placeholder="Ciudad *" value={form.ciudad} onChange={up("ciudad")}/>
            <input style={si} placeholder="Nº de comunidades que gestionas *" type="number" min="1" value={form.comunidades} onChange={up("comunidades")}/>
            <p style={{fontSize:11,color:V.mut,marginBottom:14,lineHeight:1.6}}>
              Al enviar aceptas nuestra <Link href="/politica-privacidad" style={{color:V.verde}}>política de privacidad</Link>. Datos en Alemania conforme al RGPD.
            </p>
            <button onClick={submit} disabled={!ok} style={{width:"100%",padding:"15px 0",borderRadius:100,border:"none",cursor:ok?"pointer":"default",fontWeight:700,fontSize:16,background:ok?V.verde:"#ccc",color:V.w,fontFamily:"system-ui"}}>
    ...
    <>
      ...
      <section style={{padding:"64px 20px 80px",background:`linear-gradient(170deg,${V.pale} 0%,${V.bg} 65%)`}}>
        <div style={{maxWidth:960,margin:"0 auto",textAlign:"center"}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:7,background:V.pale,border:`1px solid ${V.borde}`,borderRadius:100,padding:"6px 18px",fontSize:12,fontWeight:700,color:V.verde,marginBottom:24,textTransform:"uppercase",letterSpacing:".09em"}}>
            ...
            El único asistente IA para fincas con guardián RGPD integrado
          </div>
          ...
          <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap",marginBottom:14}}>
            <button onClick={()=>setModal(true)} style={{background:V.verde,color:V.w,border:"none",borderRadius:100,padding:"17px 38px",fontSize:17,fontWeight:700,cursor:"pointer",fontFamily:"system-ui",boxShadow:"0 6px 24px rgba(26,107,66,.35)"}}>
              Empezar 14 días gratis
            </button>
            <Link href="/como-funciona" style={{background:V.w,color:V.tx,border:`2px solid ${V.borde}`,borderRadius:100,padding:"15px 28px",fontSize:16,fontWeight:600,textDecoration:"none",display:"inline-flex",alignItems:"center"}}>
              Ver cómo funciona →
            </Link>
          </div>
      ...
      <section style={{padding:"80px 20px",background:V.w}}>
        <div style={{maxWidth:900,margin:"0 auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:20}}>
            {[
              {ic:"💬",t:"WhatsApp 24/7",d:"Responde a los vecinos al instante con la LPH verificada. Tú solo recibes lo que requiere tu criterio.",href:"/como-funciona",bg:V.pale,bc:V.borde},
              {ic:"📋",t:"Actas desde foto",d:"Foto o audio del borrador. Acta estructurada con acuerdos, votaciones y tareas en minutos.",href:"/actas",bg:"#EEF2FF",bc:"#C7D2FE"},
              {ic:"🛡️",t:"Guardián RGPD",d:"Detecta datos personales en el grupo en tiempo real y avisa antes de que el daño esté hecho.",href:"/rgpd",bg:"#D1FAE5",bc:"#6EE7B7"},
              {ic:"📁",t:"Expediente digital",d:"Historial completo de cada comunidad: actas, incidencias, morosos y comunicaciones.",href:"/como-funciona",bg:"#FEF3C7",bc:"#FDE68A"},
            ].map((b,i)=>(
              <Link key={i} href={b.href} style={{background:V.alt,border:`1px solid ${V.borde}`,borderRadius:20,padding:"28px 24px",textDecoration:"none",display:"block",transition:"box-shadow .2s"}}
                onMouseEnter={e=>e.currentTarget.style.boxShadow="0 8px 28px rgba(0,0,0,.09)"}
                onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
                <div style={{width:52,height:52,borderRadius:14,background:b.bg,border:`2px solid ${b.bc}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,marginBottom:16}}>{b.ic}</div>
      ...
      <section style={{padding:"80px 20px",background:V.alt}}>
        <div style={{maxWidth:800,margin:"0 auto",textAlign:"center"}}>
          ...
          <p style={{fontSize:16,color:V.sub,maxWidth:500,margin:"0 auto 44px",lineHeight:1.75}}>Escribe como si fueras un vecino real.</p>
          <Chat/>
        </div>
      </section>
      ...
      <section style={{padding:"80px 20px",background:V.w}}>
        <div style={{maxWidth:720,margin:"0 auto"}}>
          ...
          <div style={{background:V.alt,borderRadius:20,padding:"36px 32px",border:`1px solid ${V.borde}`,marginBottom:28}}>
            <p style={{fontSize:16,color:V.sub,lineHeight:1.95,marginBottom:18}}>Trabajando durante años junto a numerosos despachos de administración de fincas, vi siempre lo mismo: teléfonos que no paraban de sonar, grupos de WhatsApp con cientos de mensajes sin control, y vecinos llamando enfadados por los mismos problemas una y otra vez.</p>
            <p style={{fontSize:16,color:V.sub,lineHeight:1.95,marginBottom:18}}>Esa presión constante generaba un estrés enorme en administradores y empleados. El trabajo que realmente importa —gestionar, negociar, resolver, visitar comunidades— quedaba enterrado bajo una avalancha de mensajes repetitivos.</p>
            <p style={{fontSize:16,color:V.sub,lineHeight:1.95}}>De ahí nació AfincalIA. No como un producto tecnológico más, sino como una solución real a un problema que vi con mis propios ojos. Con una sola obsesión: que el administrador recupere su tiempo y la tranquilidad de su despacho.</p>
          </div>
      ...
      <section style={{padding:"96px 20px",background:"#0F2D1A",textAlign:"center"}}>
        <div style={{maxWidth:600,margin:"0 auto"}}>
          ...
          <h2 style={{fontFamily:"Georgia,serif",fontSize:"clamp(32px,5vw,52px)",color:V.w,lineHeight:1.08,marginBottom:22}}>
            Contrata tu empleado<br/>digital ya
          </h2>
          <p style={{fontSize:17,color:"rgba(255,255,255,.6)",marginBottom:44,lineHeight:1.8}}>
            Configúralo en menos de una hora. Tus vecinos siguen usando WhatsApp como siempre.
          </p>
          <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap"}}>
            <button onClick={()=>setModal(true)} style={{background:V.verde,color:V.w,border:"none",borderRadius:100,padding:"18px 44px",fontSize:17,fontWeight:800,cursor:"pointer",fontFamily:"system-ui",boxShadow:"0 8px 32px rgba(26,107,66,.4)"}}>
              Empezar 14 días gratis →
            </button>
